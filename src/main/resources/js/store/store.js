...
export default new Vuex.Store({
    ...
    actions: {
        ...
        async loadPageActionLenta({commit, state}) {

                    const response = await messagesApi.pageLenta(state.currentPage + 1)
                    const data = await response.json()
        ...
        async loadLenta({commit, state}){
            state.where = true
            state.messages = []



            const response = await messagesApi.pageLenta(0)
            const data = await response.json()
        ...
        async checkSubIsActive({commit, state}, message){
